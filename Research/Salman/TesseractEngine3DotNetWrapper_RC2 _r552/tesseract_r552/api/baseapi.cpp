...
// unsigned char packed with the MSB of the first unsigned char being the first pixel, and a
...
void TessBaseAPI::SetImage(const unsigned char* imagedata,
  ...
  if (InternalSetImage())
  {
	  ...
	  if (GetIntVariable("tessedit_thresholding_method", &threshold_method))
	  {
		  thresholder_->SetThresholdMethod(threshold_method);		  
	  }
