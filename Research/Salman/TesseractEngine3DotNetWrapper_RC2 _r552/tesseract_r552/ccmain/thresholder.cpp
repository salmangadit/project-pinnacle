...
ImageThresholder::ImageThresholder()
  : pix_(NULL),
    ...
    scale_(1), yres_(300) {
		...
		// default is TM_ISODATA_LOCAL_ADAPTIVE_THRESHOLDING
		threshold_method_ = TM_ISODATA_LOCAL_ADAPTIVE_THRESHOLDING;
    ...
    if (image_bytespp_ == 0) {
      ...
      *pix = GetPixRect();
      ...
      if (image_bytespp_ == 4) {
		...
		switch (threshold_method_)
		{
		case TM_OTSU_THRESHOLDING:
			OtsuThresholdRectToPix(reinterpret_cast<const uinT8*>(data),
				image_bytespp_, image_bytespl_, pix);
			break;
		case TM_ISODATA_LOCAL_ADAPTIVE_THRESHOLDING:
			IsodataLocalAdaptiveThresholdRectToPix(reinterpret_cast<const uinT8*>(data),
				image_bytespp_, image_bytespl_, pix);
			break;
		case TM_SAUVOLA_LOCAL_ADAPTIVE_THRESHOLDING:
			SauvolaLocalAdaptiveThresholdRectToPix(reinterpret_cast<const uinT8*>(data),
				image_bytespp_, image_bytespl_, pix);
			break;
		default:
			OtsuThresholdRectToPix(reinterpret_cast<const uinT8*>(data),
		...
		case TM_OTSU_THRESHOLDING:
			...
			break;
		case TM_ISODATA_LOCAL_ADAPTIVE_THRESHOLDING:
			IsodataLocalAdaptiveThresholdRectToPix(temp_image.get_buffer(),
                               image_bytespp_,
                               COMPUTE_IMAGE_XDIM(temp_image.get_xsize(),
                                                  temp_image.get_bpp()),
                               pix);
			break;
		case TM_SAUVOLA_LOCAL_ADAPTIVE_THRESHOLDING:
			SauvolaLocalAdaptiveThresholdRectToPix(temp_image.get_buffer(),
                               image_bytespp_,
                               COMPUTE_IMAGE_XDIM(temp_image.get_xsize(),
                                                  temp_image.get_bpp()),
                               pix);
    ...
    // Threshold grey or color.
	  switch (threshold_method_)
		...
		case TM_OTSU_THRESHOLDING:
			...
			break;
		case TM_ISODATA_LOCAL_ADAPTIVE_THRESHOLDING:
			IsodataLocalAdaptiveThresholdRectToPix(image_data_, image_bytespp_, image_bytespl_, pix);
			break;
		case TM_SAUVOLA_LOCAL_ADAPTIVE_THRESHOLDING:
			SauvolaLocalAdaptiveThresholdRectToPix(image_data_, image_bytespp_, image_bytespl_, pix);
			break;
		default:
			OtsuThresholdRectToPix(image_data_, image_bytespp_, image_bytespl_, pix);
			break;
...
/// Isodata Local adaptive threshold the rectangle, taking everything except the image buffer
/// pointer from the class, to the output Pix.
void ImageThresholder::IsodataLocalAdaptiveThresholdRectToPix(const unsigned char* imagedata,
                              int bytes_per_pixel, int bytes_per_line,
							  Pix** pix) const {
	// binary image
	*pix = pixCreate(rect_width_, rect_height_, 1);
	...
	int kernelWidth = 11;
	int kernelHeight = 11;
	LocalAdaptiveThreshold::Isodata(
...
/// Sauvola Local adaptive threshold the rectangle, taking everything except the image buffer
/// pointer from the class, to the output Pix.
void ImageThresholder::SauvolaLocalAdaptiveThresholdRectToPix(const unsigned char* imagedata,
                              int bytes_per_pixel, int bytes_per_line,
							  Pix** pix) const {
	// binary image
	*pix = pixCreate(rect_width_, rect_height_, 1);
	...
	int kernelWidth = 11;
	int kernelHeight = 11;
	LocalAdaptiveThreshold::Sauvola(
		imagedata, bytes_per_pixel, bytes_per_line,
		rect_left_, rect_top_, rect_width_, rect_height_,
		kernelWidth, kernelHeight,
		pixdata, wpl);
